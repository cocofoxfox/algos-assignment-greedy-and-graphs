...
public class FastestRoutePublicTransit {
      ...
      for (int v = 0; v < numVertices; v++) {
        ...
        if (!processed[v] && lengths[u][v] != 0 && times[u] != Integer.MAX_VALUE) {
          ...
          if (firstTrainTime >= arriveTime) {
            ...
            if ((arriveTime - firstTrainTime) % freq[u][v] == 0)
              waitingTime = 0;
            else
  ...
  public static void main(String[] args) {
    ...
    int start = 0;
    int end = 3;
    int[][] first = new int[][] { { 0, 5, 0, 0 }, { 5, 0, 6, 0 }, { 0, 6, 0, 7 }, { 0, 0, 7, 0 } };

    int[][] lengths = new int[][] { { 0, 3, 0, 0 }, { 3, 0, 4, 0 }, { 0, 4, 0, 5 }, { 0, 0, 5, 0 } };

    int[][] freq = new int[][] { { 0, 1, 0, 0 }, { 1, 0, 2, 0 }, { 0, 2, 0, 3 }, { 0, 0, 3, 0 } };
